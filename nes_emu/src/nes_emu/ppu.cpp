...
void PPU::evaluate_scanline_sprites(PictureBus& bus) {
    ...
    for (NES_Byte i = sprite_data_address / 4; i < 64; ++i) {
        ...
        if (0 <= diff && diff < range) {
            ...
            if (scanline_sprite_count >= scanline_max_sprites)
                break;
        }
    }
...
void PPU::set_parameter(PARAM key, int value) {
    switch (key) {
    case PARAM::UNLIMIT_SPRITE:  scanline_max_sprites = value ? EXTRA_MAX_SCANLINE_SPRITES : MAX_SCANLINE_SPRITES; break;
    }
}
